...
export function BookingSteps({ currentStep, steps = defaultSteps }: BookingStepsProps) {
  return (
    <div className="bg-white dark:bg-[#18181b] rounded-2xl border border-slate-200 dark:border-[#27272a] p-8 mb-8 transition-colors duration-300 shadow-sm">
        ...
        {steps.map((step, index) => {
          ...
          return (
            <div key={step.key} className="relative z-10 flex flex-col items-center group cursor-default bg-white dark:bg-[#18181b] px-4 -mx-4 transition-colors duration-300">
               {/* Background masking helper above is trying to hide line but z-index issues. 
                   Better approach: Just put the steps on top. 
                   The line is z-0. Steps are z-10. 
               */}
              <div 
                className={`
                  w-12 h-12 rounded-full flex items-center justify-center text-lg transition-all duration-500 border-2
